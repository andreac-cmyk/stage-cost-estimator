...
# ── Page Config ──────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="Netflix Studios Stage Estimator",
    page_icon="🎬",
    layout="wide",
...
<style>
    ...
    [data-testid="stSidebar"] .stTextInput label {
        color: #aaa !important; font-size: 0.8rem; text-transform: uppercase;
    ...
    .section-subheader { font-size: 0.85rem; color: #888; margin: -0.5rem 0 1rem 0; }
    .top-banner {
        background: linear-gradient(135deg, #B20710 0%, #E50914 50%, #B20710 100%);
        color: white; padding: 1.2rem 1.5rem; border-radius: 12px; margin-bottom: 1.5rem;
    }
    ...
    .loc-card .sub { font-size: 0.78rem; color: #888; margin-top: 4px; }
    .badge { display: inline-block; padding: 2px 10px; border-radius: 20px;
        font-size: 0.7rem; font-weight: 600; margin-top: 8px; text-transform: uppercase; }
    ...
    .predict-card {
        ...
        border: 2px solid #3b82f6; border-radius: 16px; padding: 20px 28px; margin-bottom: 1rem;
    }
    .predict-card h3 { font-size: 1rem; font-weight: 600; color: #1e40af; margin: 0 0 8px 0; }
    .predict-card .pred-val { font-size: 0.9rem; color: #1e3a5f; }
    .rate-note {
        background: #f8f9fa; border-left: 3px solid #E50914; padding: 12px 16px;
        border-radius: 0 8px 8px 0; font-size: 0.78rem; color: #6b7280;
...
LOCATION_ROUTING = {
    ...
    "UCAN Scripted Series": ["Los Angeles", "Albuquerque", "Atlanta", "New York", "New Jersey", "UK", "Canada"],
    "Film": ["Los Angeles", "Albuquerque", "Atlanta", "New York", "New Jersey", "UK", "Canada"],
    "Animation Series": ["Los Angeles", "Albuquerque", "Atlanta", "New York", "New Jersey", "UK", "Canada"],
    ...
    "APAC": ["Japan"],  # default, overridden per buying team below
...
TEAM_LOCATION_OVERRIDES = {
    # APAC teams
    "Japan": ["Japan"],
    "Korea": ["Japan"],  # K-Art studios (Japan region for now)
    "Anime": ["Japan"],
    ...
    "Canada": ["Canada", "Los Angeles", "New York", "UK"],
}
...
FRANCHISE_LOCATIONS = {
    ...
    "Squid Game": {"region": "Japan", "facility": "Toho Studios"},
    "Hierarchy": {"region": "Japan", "facility": "K-ART Studio"},
}
...
TITLE_LOCATION_HINTS = {
    ...
    "Japan": [
        ...
        "sakura", "edo", "kyoto", "osaka", "okinawa", "squid game", "korean",
        "k-drama", "seoul", "busan", "parasyte", "alice in borderland",
    ],
    "Madrid": [
...
def get_title_location_hints(title):
    ...
    if "facility" in title_data and title_data.get("pod"):
        ...
        pod_to_region = {
            ...
            "UCAN East": "Atlanta",  # Cinespace Atlanta primarily
            "UCAN West": "Albuquerque",  # or Canada depending on facility
            "UK": "UK",
            "Spain": "Madrid",
            "APAC": "Japan",
        }
        ...
        elif "Canadian" in facility or "Toronto" in facility or "Pinewood Toronto" in facility: hints["Canada"] = hints.get("Canada", 0) + 3
        elif "Cinespace Atlanta" in facility or "Martini" in facility: hints["Atlanta"] = hints.get("Atlanta", 0) + 3
        ...
        elif "Toho" in facility or "K-ART" in facility or "YCDSMC" in facility: hints["Japan"] = hints.get("Japan", 0) + 3
...
def score_regions(results, relevant_regions, prod_type_key, franchise_info, buying_org, title_hints=None):
    ...
    for region, r in valid_results.items():
        ...
        # 2. Prior season / franchise loyalty (25% weight)
        if franchise_info and franchise_info["region"] == region:
            score += 25
            reasons.append(f"Prior seasons shot here ({franchise_info['franchise']})")

        # 3. LA affinity for talent-heavy productions (15% weight)
        if region == "Los Angeles":
            la_bonus = prod_type["la_affinity"] * 15
            score += la_bonus
            if prod_type["la_affinity"] >= 0.7:
                reasons.append("Talent base & crew in LA")
        elif region in ("New York", "New Jersey"):
            # NY/NJ also have talent pools
            score += prod_type["la_affinity"] * 5
        
        # 4. Tax incentive value (10% weight)
        if r["incentive_rate"] > 0:
            incentive_score = r["incentive_rate"] / 0.35  # normalize to max 35%
            score += min(incentive_score * 10, 10)
            if r["incentive_rate"] >= 0.25:
                reasons.append(f"{r['incentive_rate']:.0%} tax incentive")

        # 5. Stage availability (5% weight) — more options is better
        avail_score = min(r["available"] / 10, 1)  # normalize, cap at 10
        score += avail_score * 5
        if r["available"] >= 8:
            reasons.append(f"{r['available']} stages available")

        # 6. Crew cost advantage (5% weight)
        if r["crew_idx"] < 1.0:
            score += (1 - r["crew_idx"]) * 50  # up to 5 points
            reasons.append(f"Lower crew costs ({r['crew_idx']:.0%} of LA)")

        # 7. Title keyword hints (up to 15 bonus points)
        if title_hints and region in title_hints:
            hint_score = min(title_hints[region] * 5, 15)
            score += hint_score
            reasons.append(f"Title suggests {region} (keyword match)")
...
STAGES = [
    ...
    # Atlanta - Martini
    ("Martini Studios", "Atlanta", "Stage 01", 11600, 1040),
    ("Martini Studios", "Atlanta", "Stage 02", 18000, 1615),
    ("Martini Studios", "Atlanta", "Stage 03", 15600, 1400),
    ("Martini Studios", "Atlanta", "Stage 04", 15600, 1400),
    ("Martini Studios", "Atlanta", "Stage 05", 15600, 1400),
    ("Martini Studios", "Atlanta", "Stage 06", 15600, 1400),
    ("Martini Studios", "Atlanta", "Stage 07", 23650, 2122),
    ("Martini Studios", "Atlanta", "Stage 08", 24950, 2238),
    # Canada
    ("Canadian Motion Picture Park", "Canada", "CMPP1 - Stage 01", 21200, 4734),
    ("Canadian Motion Picture Park", "Canada", "CMPP1 - Stage 02", 17000, 3796),
    ("Canadian Motion Picture Park", "Canada", "CMPP1 - Stage 03", 16900, 3774),
    ("Canadian Motion Picture Park", "Canada", "CMPP1 - Stage 04", 9184, 2051),
    ("Canadian Motion Picture Park", "Canada", "Pacific Studios - Stage 01", 36000, 8038),
    ("Canadian Motion Picture Park", "Canada", "Pacific Studios - Stage 02", 23000, 5136),
    # Madrid
...
MARKET = {
    ...
    "Canada":       {"third_party_rate": 0.22, "incentive": 0.25, "crew_idx": 0.90, "per_diem": 250, "travel": 600},
...
with st.sidebar:
    ...
    if title_data:
        # Determine buying org from audience country/language
        ac = title_data.get("audience_country", "US")
        lang = title_data.get("language", "en")
        if title_data.get("is_animated"):
    ...
    if buying_org:
        buying_team = st.selectbox("Buying Team", options=BUYING_ORGS[buying_org])
    else:
    ...
    if franchise_info:
        st.markdown(f"""
        <div style="background: #ede9fe; border: 1px solid #8b5cf6; border-radius: 8px; padding: 12px; margin: 8px 0;">
            <div style="font-weight: 600; color: #5b21b6; font-size: 0.85rem;">🔄 Returning Series</div>
            <div style="color: #6d28d9; font-size: 0.8rem;">
    ...
    if prediction:
        ...
        <div class="predict-card">
            ...
            <div class="pred-val">
                <strong>{prediction['stages']} stages</strong> · {prediction['duration_weeks']} weeks · {prediction['sqft']:,} sq ft<br>
                <span style="font-size:0.75rem; color:#6b7280;">{prediction['source']}</span>
            </div>
        </div>
        """, unsafe_allow_html=True)

    st.markdown("<div style='color:#888; font-size:0.7rem; text-transform:uppercase; letter-spacing:0.1em; margin-bottom:0.5rem;'>Production Parameters</div>", unsafe_allow_html=True)
...
<div class="top-banner" style="display:flex; align-items:center; justify-content:space-between;">
    <div>
        <div style="font-size:0.7rem; text-transform:uppercase; letter-spacing:0.15em; color:rgba(255,255,255,0.7); margin-bottom:2px;">Netflix Studios</div>
        <h1 style="margin:0;">Stage Estimator</h1>
...
if buying_org:
    ...
    relevant_regions = ["Los Angeles", "Albuquerque", "Atlanta", "New York", "New Jersey", "UK", "Canada"]
...
if valid and best_region:
    ...
    with rec_col1:
        ...
        st.markdown(f"""
        <div class="rec-card" style="padding:16px 20px;">
            <div style="font-size:0.75rem; color:#15803d; text-transform:uppercase; letter-spacing:0.05em;">Recommended</div>
            ...
            <div style="font-size:0.75rem; color:#4b5563; margin-top:8px; line-height:1.5;">{reasons_html}</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with rec_col2:
        if runner_up:
            ...
            st.markdown(f"""
            <div style="background:#f8f9fa; border:1px solid #e5e7eb; border-radius:12px; padding:16px 20px; height:100%;">
                <div style="font-size:0.7rem; color:#888; text-transform:uppercase; letter-spacing:0.05em;">Runner-Up</div>
                ...
                <div style="font-size:0.72rem; color:#6b7280; margin-top:6px; line-height:1.4;">{ru_reasons}</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        if third_place:
            ...
            st.markdown(f"""
            <div style="background:#f8f9fa; border:1px solid #e5e7eb; border-radius:12px; padding:12px 20px; margin-top:8px;">
                <div style="font-size:0.7rem; color:#888; text-transform:uppercase;">3rd Option</div>
    ...
    with rec_col3:
        m1, m2 = st.columns(2)
        with m1:
            st.metric("Net Cost to Title", fmt(best["cost_to_title_nflx_net"]))
            st.metric("Per Episode", fmt(best["per_episode"]))
        with m2:
            st.metric("Netflix Impact", "$0", "Net Neutral")
            st.metric("If 3P Instead", f"+{fmt(best['incremental_vs_nflx'])}", "Incremental", delta_color="inverse")
...
if available_regions:
    ...
    if len(region_facilities) > 1:
        ...
        if selected_facility != "All Facilities":
            ...
            if len(facility_stages) > 0:
                with loc_col2:
                    st.markdown(f"""
                    <div style="background:#eff6ff; border-radius:8px; padding:10px 16px; margin-top:24px; font-size:0.82rem; color:#1e40af;">
    ...
    <div style="display:flex; gap:16px; margin-bottom:16px;">
        <div style="flex:1; background:linear-gradient(135deg,#f0fdf4,#dcfce7); border:2px solid #22c55e; border-radius:12px; padding:20px; text-align:center;">
            <div style="font-size:0.75rem; color:#15803d; text-transform:uppercase; letter-spacing:0.08em; font-weight:600;">Netflix Stage</div>
            ...
            <div style="font-size:0.75rem; color:#4b5563;">Internal transfer · Net neutral to P&L</div>
        </div>
        <div style="flex:0.5; display:flex; align-items:center; justify-content:center;">
            <div style="text-align:center; padding:8px;">
                <div style="font-size:0.65rem; color:#888; text-transform:uppercase; letter-spacing:0.1em;">Netflix saves</div>
                <div style="font-size:2.8rem; font-weight:900; color:#E50914; line-height:1.1;">{fmt(savings_amt)}</div>
                <div style="font-size:0.72rem; color:#6b7280; margin-top:4px;">by using MLA stage<br>vs 3rd party</div>
        ...
        <div style="flex:1; background:linear-gradient(135deg,#fff5f5,#fee2e2); border:2px solid #fca5a5; border-radius:12px; padding:20px; text-align:center;">
            <div style="font-size:0.7rem; color:#991b1b; text-transform:uppercase; letter-spacing:0.1em; font-weight:600;">⚠ 3rd Party Stage</div>
            ...
            <div style="font-size:0.68rem; color:#6b7280; margin-top:4px; line-height:1.4;">3P lease paid externally<br>+ Netflix stage sits vacant (overhang)</div>
            <div style="border-top:2px solid #fca5a5; margin:10px 0; padding-top:10px;">
                ...
                <div style="font-size:0.68rem; color:#6b7280;">cost to title</div>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
        ...
        with cols[i]:
            if r["available"] > 0:
                ...
                <div class="loc-card {card_class}">
                    <h3>{region} <span style="font-size:0.75rem; color:#888;">({score_val:.0f} pts)</span></h3>
                    ...
                    <div class="sub">3P alt: {fmt(r['incremental_vs_nflx'])} incremental (unused stage + 3P lease)</div>
                    <div class="sub">{r['available']} stages · {r['incentive_rate']:.0%} incentive</div>
                    <div class="sub" style="text-align:left; color:#4b5563; font-size:0.72rem; margin-top:6px;">{reasons_html}</div>
                    {badge}
                </div>
                """, unsafe_allow_html=True)
            else:
...
if valid and best_region:
    ...
    for region in sorted_regions_chart:
        ...
        chart_data.append({
            ...
            "3P Incremental to Netflix": r["incremental_vs_nflx"],
        })
    ...
    fig.add_trace(go.Bar(
        name="3P Incremental: Vacant Netflix Stage Cost + 3rd Party Lease",
        x=chart_df["Location"], y=chart_df["3P Incremental to Netflix"],
        marker_color="#dc2626",
        text=[fmt(v) for v in chart_df["3P Incremental to Netflix"]],
        textposition="auto", textfont=dict(size=11)))
    fig.update_layout(
        barmode="group",
        title=dict(text="Netflix Stage (Net Neutral) vs 3P (Vacant Stage Cost + External Lease)", font=dict(size=16, family="Inter")),
...
<div class="rate-note">
    ...
    <br><br>
    <strong>Prediction Model:</strong> Stage count and duration predictions are based on analysis of 488 historical
    productions across all Netflix pods. Title-specific data overrides pod-level averages when available.
</div>
""", unsafe_allow_html=True)
